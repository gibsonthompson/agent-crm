...
const SMS_TEMPLATES = [
  { label: 'New lead intro', text: "Hi {name}, this is {agent} — I received your inquiry and would love to help! When is a good time to chat?" },
  { label: 'Showing follow-up', text: "Hi {name}, thanks for viewing the property today! What did you think? Let me know if you'd like to see more options." },
  { label: 'Offer submitted', text: "Hi {name}, great news — your offer has been submitted! I'll keep you posted as soon as I hear back." },
  { label: 'Under contract', text: "Hi {name}, congratulations! Your offer was accepted and you're officially under contract!" },
  { label: 'Listing follow-up', text: "Hi {name}, just checking in on your listing timeline. I'm here whenever you're ready." },
  { label: 'Open house follow-up', text: "Hi {name}, thanks for visiting the open house! Any questions? Happy to schedule a private showing." },
  { label: 'Price reduction', text: "Hi {name}, a property you liked just had a price reduction. Want me to send updated details?" },
  { label: 'Closing reminder', text: "Hi {name}, just a reminder — your closing is scheduled for {date}. Let me know if you need anything!" },
  { label: 'Review request', text: "Hi {name}, congrats on your new home! If you had a great experience, I'd really appreciate a Google review!" },
  { label: 'Anniversary', text: "Hi {name}, happy home-iversary! Hope you're loving it. Let me know if you ever need anything!" },
]
const HELP_SECTIONS = [
  { title: 'Quick actions', body: 'The buttons at the top let you call, text, or email the client. Each opens right from this page.' },
  { title: 'Pipeline stage', body: "Change the lead's stage using the status pills. Moving to Lost will ask for a reason. The recommended action updates based on current status." },
  { title: 'Lead details', body: 'Track lead type (buyer/seller), budget range, preferred areas, lead source, and property interest. These help you personalize outreach.' },
  { title: 'Scheduling', body: 'Set a date and time for showings, consultations, or closings. This makes the contact show up on the Calendar page.' },
  { title: 'Follow-up date', body: "Set a follow-up date to remind yourself to check back. Overdue follow-ups show in the Needs Attention section on the Dashboard." },
  { title: 'Notes & Activity', body: 'Use notes for anything about this client. The activity timeline logs status changes, emails, and texts automatically.' },
  { title: 'Remember to save', body: 'After editing any field, tap the blue Save button at the top right. Changes are not saved automatically.' },
]
...
export default function ContactDetailPage() {
  ...
  const resolveVariables = (text) => text.replace(/\{name\}/g, contact.name.split(' ')[0]).replace(/\{agent\}/g, user?.name?.split(' ')[0] || 'Agent').replace(/\{date\}/g, formData.scheduled_date ? new Date(formData.scheduled_date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }) : 'TBD')
