    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-5">
          {sections.map((s, i) => (<div key={i}><h4 className="text-sm font-semibold text-gray-800 mb-1">{s.title}</h4><p className="text-sm text-gray-600 leading-relaxed">{s.body}</p></div>))}
        </div>
        <div className="p-5 border-t border-gray-100"><button onClick={onClose} className="w-full py-3 bg-[#1a2e44] text-white rounded-xl font-semibold hover:bg-[#0f1d2d] transition-colors">Got it</button></div>
      </div>
    </div>
  )
}
