...
export default function AdminDashboard() {
  ...
  const handleAddNote = async () => {
    ...
    try {
      const r = await fetch('/api/admin/notes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: newNoteText }) })
  ...
  const handleDeleteNote = async (id) => {
    ...
    try { await fetch('/api/admin/notes', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }) } catch (e) {}
  }

  const handleUpdateNote = async (id, text) => {
    setNotes(prev => prev.map(n => n.id === id ? { ...n, text } : n))
    setEditingNote(null)
    try { await fetch('/api/admin/notes', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, text }) }) } catch (e) {}
  }
  ...
  const pendingGCI = transactions.filter(t => t.status === 'pending').reduce((s, t) => s + (parseFloat(t.net_commission) || 0), 0)
  ...
  const upcomingMilestones = transactions.filter(t => t.status === 'pending' && t.milestones).flatMap(t => {
    ...
    return (ms || []).filter(m => !m.completed && m.due_date).map(m => ({ ...m, transaction: t })).filter(m => {
      const d = new Date(m.due_date); return d >= now && d <= new Date(now.getTime() + 7 * 864e5)
    })
  ...
  return (
    <div className="px-4 py-4 sm:py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg sm:text-2xl font-bold text-[#1a2e44]">{greeting}, {user?.name?.split(' ')[0]}</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-6">
        <div className="bg-white rounded-xl p-3 shadow-sm"><p className="text-[10px] text-gray-500 uppercase">Pending GCI</p><p className="text-lg font-bold text-[#e8963e]">{fmtMoney(pendingGCI)}</p><p className="text-[10px] text-gray-400">{activeDeals} active</p></div>
        <div className="bg-white rounded-xl p-3 shadow-sm"><p className="text-[10px] text-gray-500 uppercase">Closed YTD</p><p className="text-lg font-bold text-green-600">{fmtMoney(closedGCI)}</p><p className="text-[10px] text-gray-400">{closedDeals} closed</p></div>
      ...
      <div className="bg-white rounded-xl shadow-sm mb-4 overflow-hidden">
        <div className="px-4 sm:px-6 py-3 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-2"><div className="w-2 h-2 bg-[#1a2e44] rounded-full" /><h3 className="text-sm font-semibold text-gray-800">Today</h3>{todayEvents.length > 0 && <span className="text-xs text-gray-400">({todayEvents.length})</span>}</div>
          <Link href="/admin/calendar" className="text-xs text-[#1a2e44] font-medium">Calendar</Link>
        </div>
        {todayEvents.length === 0 ? (
          <div className="p-6 text-center"><p className="text-sm text-gray-400">Nothing scheduled today</p></div>
        ) : (
          <div className="divide-y divide-gray-100">{todayEvents.map(item => (
            <Link key={item.id} href={'/admin/contacts/' + item.id} className="flex items-center justify-between p-4 sm:px-6 hover:bg-gray-50">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-0.5"><p className="font-semibold text-gray-900 text-sm truncate">{item.name}</p><span className={'inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium ' + (STATUS_COLORS[item.status] || '')}>{STATUS_LABELS[item.status]}</span></div>
                <p className="text-xs text-gray-500">{item.scheduled_time && <span className="font-medium">{item.scheduled_time}</span>}{item.scheduled_time && item.service_type && ' · '}{item.service_type}</p>
      ...
      {needsAttention.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm mb-4 overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">{needsAttention.slice(0, 5).map(item => {
            ...
            const hours = Math.floor((Date.now() - new Date(item.created_at)) / 36e5)
            ...
            return (
              <Link key={item.id} href={'/admin/contacts/' + item.id} className="flex items-center justify-between p-4 sm:px-6 hover:bg-gray-50">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-0.5"><p className="font-semibold text-gray-900 text-sm truncate">{item.name}</p><span className={'inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium ' + (STATUS_COLORS[item.status] || '')}>{STATUS_LABELS[item.status]}</span></div>
                  <p className={'text-xs ' + (isNew && hours > 24 ? 'text-red-600' : 'text-amber-600')}>{reason}</p>
                </div>
      ...
      {upcomingMilestones.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm mb-4 overflow-hidden">
          <div className="px-4 sm:px-6 py-3 border-b border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-2"><svg className="w-4 h-4 text-[#e8963e]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg><h3 className="text-sm font-semibold text-gray-800">This Week</h3></div>
            <Link href="/admin/transactions" className="text-xs text-[#1a2e44] font-medium">Deals</Link>
          </div>
          <div className="divide-y divide-gray-100">{upcomingMilestones.slice(0, 5).map((m, i) => {
            const du = Math.ceil((new Date(m.due_date) - now) / 864e5)
            return (
              <Link key={i} href="/admin/transactions" className="flex items-center justify-between p-4 sm:px-6 hover:bg-gray-50">
                ...
                <span className={'text-xs font-medium px-2.5 py-1 rounded-full ' + (du <= 2 ? 'bg-red-100 text-red-700' : du <= 5 ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700')}>{du === 0 ? 'Today' : du === 1 ? 'Tomorrow' : du + 'd'}</span>
              </Link>
            )
          })}</div>
        </div>
      ...
      {todayEvents.length === 0 && needsAttention.length === 0 && upcomingMilestones.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm p-8 text-center mb-4">
          <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-3"><svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></div>
          <p className="font-semibold text-gray-800">All clear</p>
          <p className="text-sm text-gray-500 mt-1">Nothing scheduled and no leads need attention.</p>
        </div>
      )}

      {/* 4. NOTES — small sticky boxes */}
      <div className="bg-white rounded-xl shadow-sm overflow-hidden mb-4">
        <div className="px-4 sm:px-6 py-2.5 border-b border-gray-100 flex items-center justify-between">
          ...
          <button onClick={() => setShowAddNote(!showAddNote)} className="text-[10px] font-medium text-[#1a2e44] hover:underline">+ Add</button>
        </div>
        {showAddNote && (
          <div className="px-4 sm:px-6 py-2.5 border-b border-gray-100 bg-gray-50">
            <div className="flex gap-2">
              <input type="text" value={newNoteText} onChange={(e) => setNewNoteText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddNote()} placeholder="Type a note..." style={{ fontSize: '16px' }} className="flex-1 px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#1a2e44] outline-none" autoFocus />
              <button onClick={handleAddNote} disabled={!newNoteText.trim()} className="px-3 py-1.5 bg-[#1a2e44] text-white text-xs font-medium rounded-lg disabled:opacity-50">Add</button>
            </div>
          </div>
        )}
        {notes.length === 0 && !showAddNote ? (
          <div className="p-4 text-center"><p className="text-xs text-gray-300">No notes</p></div>
        ) : (
          <div className="p-3 flex flex-wrap gap-2">{notes.map(n => (
            <div key={n.id} className="relative group rounded-lg border border-amber-200 bg-amber-50 px-3 py-1.5 max-w-[220px]">
              {editingNote === n.id ? (
                <input type="text" defaultValue={n.text} autoFocus onBlur={(e) => handleUpdateNote(n.id, e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleUpdateNote(n.id, e.target.value) }} className="w-full bg-transparent text-xs outline-none text-amber-900" />
              ) : (
                <p onClick={() => setEditingNote(n.id)} className="text-xs cursor-text text-amber-900">{n.text}</p>
              )}
