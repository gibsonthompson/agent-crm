...
const COLUMNS = [
  ...
  { key: 'contacted', label: 'Contacted', color: 'border-yellow-400', bg: 'bg-yellow-50', badge: 'bg-yellow-100 text-yellow-700', help: 'You\'ve spoken with them. Schedule a showing or consultation next.' },
  { key: 'showing_scheduled', label: 'Showing', color: 'border-purple-400', bg: 'bg-purple-50', badge: 'bg-purple-100 text-purple-700', help: 'A showing or consultation is scheduled. Prepare your CMA or property list.' },
  { key: 'offer_submitted', label: 'Offer Sent', color: 'border-indigo-400', bg: 'bg-indigo-50', badge: 'bg-indigo-100 text-indigo-700', help: 'Offer submitted, waiting on response. Follow up with listing agent.' },
  { key: 'under_contract', label: 'Contract', color: 'border-emerald-400', bg: 'bg-emerald-50', badge: 'bg-emerald-100 text-emerald-700', help: 'Under contract! Create a Transaction to track milestones and deadlines.' },
  { key: 'closed', label: 'Closed', color: 'border-green-400', bg: 'bg-green-50', badge: 'bg-green-100 text-green-700', help: 'Deal closed. Add to SOI and send a review request.' },
  { key: 'lost', label: 'Lost', color: 'border-red-400', bg: 'bg-red-50', badge: 'bg-red-100 text-red-700', help: 'Didn\'t work out. You\'ll be asked for a reason.' },
]
...
export default function PipelinePage() {
  ...
  const fetchContacts = async () => {
    try { const params = user.role === 'member' ? `?user_id=${user.id}&user_role=member` : ''; const r = await fetch('/api/contact' + params); const res = await r.json(); if (res.data) setContacts(res.data) }
  ...
  const updateStatus = async (contactId, newStatus, oldStatus) => {
    ...
    try {
      ...
      setSuccessMsg('Moved to ' + (COLUMNS.find(c => c.key === newStatus)?.label || newStatus)); setTimeout(() => setSuccessMsg(''), 3000)
  ...
  const handleCloseLost = async () => {
    ...
    } catch (err) { setContacts(prev => prev.map(s => s.id === id ? { ...s, status: oldStatus } : s)) }
  }
                  ...
                  {cards.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).map((contact) => {
                    ...
                    return (
                      <div key={contact.id} onClick={() => setMoveModal(contact)} className="bg-white rounded-lg p-3 shadow-sm border border-gray-100 active:bg-gray-50">
                        ...
                        <a href={'tel:' + contact.phone} className="text-xs text-[#1a2e44] font-medium" onClick={(e) => e.stopPropagation()}>{formatPhone(contact.phone)}</a>
                      </div>
                    )
                  })}
                </div>
