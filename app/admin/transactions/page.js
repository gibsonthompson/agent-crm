...
// Auto-generated milestones from contract date (days offset)
const MILESTONE_TEMPLATES = [
  { label: 'Earnest Money Due', offset: 3, critical: true },
  { label: 'Home Inspection', offset: 10, critical: true },
  { label: 'Inspection Objection Deadline', offset: 12, critical: true },
  { label: 'Inspection Resolution', offset: 15, critical: false },
  { label: 'Appraisal Ordered', offset: 7, critical: false },
  { label: 'Appraisal Deadline', offset: 21, critical: true },
  { label: 'Loan Approval Deadline', offset: 25, critical: true },
  { label: 'Title Commitment Due', offset: 20, critical: false },
  { label: 'HOA Docs Review', offset: 14, critical: false },
  { label: 'Final Walkthrough', offset: -1, critical: true }, // -1 = 1 day before closing
...
export default function TransactionsPage() {
  ...
  const fetchTransactions = async () => {
    try { const r = await fetch('/api/admin/transactions'); const d = await r.json(); if (d.transactions) setTransactions(d.transactions) }
  ...
  const generateMilestones = (contractDate, closingDate) => {
    ...
    return MILESTONE_TEMPLATES.map(t => {
      let due
      if (t.label === 'Closing Day' && closeDate) {
        due = closeDate
      } else if (t.label === 'Final Walkthrough' && closeDate) {
        due = new Date(closeDate); due.setDate(due.getDate() - 1)
      } else {
        due = new Date(cd); due.setDate(due.getDate() + t.offset)
      }
  ...
  const toggleMilestone = (idx) => {
    setFormData(p => {
      ...
      m[idx] = { ...m[idx], completed: !m[idx].completed }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const salePrice = n(formData.sale_price)
      const grossComm = salePrice * (n(formData.commission_pct) / 100)
      const brokerCut = grossComm * (n(formData.broker_split_pct) / 100)
      const referral = n(formData.referral_fee)
      const netComm = grossComm - brokerCut - referral

      const payload = {
        ...formData,
        sale_price: salePrice,
        commission_pct: n(formData.commission_pct),
        broker_split_pct: n(formData.broker_split_pct),
      ...
      if (r.ok) { setEditing(null); fetchTransactions(); setSuccessMsg(editing === 'new' ? 'Transaction added' : 'Updated'); setTimeout(() => setSuccessMsg(''), 2000) }
    } catch (e) {} finally { setSaving(false) }
  }

  const handleDelete = async (id) => {
    if (!confirm('Delete this transaction?')) return
    try { await fetch('/api/admin/transactions', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); fetchTransactions(); setSuccessMsg('Deleted'); setTimeout(() => setSuccessMsg(''), 2000) } catch (e) {}
  }
  ...
  return (
    <div className="px-4 py-4 sm:py-8">
      ...
      <div className="flex gap-2 mb-4">
        {[{ v: 'all', l: 'All' }, { v: 'pending', l: 'Pending' }, { v: 'closed', l: 'Closed' }, { v: 'fell_through', l: 'Fell Through' }].map(f => (
          <button key={f.v} onClick={() => setFilter(f.v)} className={'px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ' + (filter === f.v ? 'bg-[#1a2e44] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}>{f.l}</button>
        ))}
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-4 border-2 border-[#1a2e44]/20">
          ...
          <div className="space-y-4">
            ...
            {formData.milestones.length > 0 && (
              <div>
                <p className="text-xs text-gray-500 mb-2">Transaction Milestones</p>
                <div className="space-y-1.5 max-h-[300px] overflow-y-auto">
                  {formData.milestones.map((m, i) => {
                    const isPast = new Date(m.due_date) < new Date() && !m.completed
                    return (
                      <button key={i} onClick={() => toggleMilestone(i)} className={'w-full flex items-center gap-3 px-3 py-2 rounded-lg border text-left text-sm transition-all ' + (m.completed ? 'bg-green-50 border-green-200 line-through text-gray-400' : isPast ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200')}>
                        <div className={'w-5 h-5 rounded flex items-center justify-center flex-shrink-0 border ' + (m.completed ? 'bg-green-500 border-green-500' : 'border-gray-300')}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={'text-sm ' + (m.completed ? 'text-gray-400' : m.critical ? 'font-medium text-gray-900' : 'text-gray-700')}>{m.label}{m.critical && !m.completed && <span className="text-[10px] text-red-500 ml-1">●</span>}</p>
                        </div>
                        <span className={'text-xs flex-shrink-0 ' + (isPast && !m.completed ? 'text-red-600 font-medium' : 'text-gray-400')}>{fmtDate(m.due_date)}</span>
                      </button>
                    )
                  })}
                </div>
